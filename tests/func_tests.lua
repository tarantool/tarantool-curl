...
function async_get(value)
    ...
    local response = nil
    if value.param then
         response = value.f(http, value.param, host, {done=dcb, write=wcb, read=rcb, ctx=ctx})
    else
         response = value.f(http, host, {done=dcb, write=wcb, read=rcb, ctx=ctx}) 
    end
...
post_requests["sync_request with POST"] = {f = http.sync_request, param = "POST", host = "https://httpbin.org/post"} 
post_requests["sync_request with PUT"] = {f = http.sync_request, param = "PUT", host = "https://httpbin.org/put"} 
post_requests["async_request with PUT"] = {f = http.request, param = "PUT", host = "https://httpbin.org/put", async=true} 
post_requests["async_request with POST"] = {f = http.request, param = "POST", host = "https://httpbin.org/post", async=true} 
...
function post_test(value)
    if value.async then
       ...
       response = value.f(http, value.param, value.host, {done=good_done_cb, write=write1, read = function(cnt, ctx1) end, headers = headers, ctx=ctx})
        while not ctx.done
            do
                fiber.sleep(0.001)
            end
        ...
        if value.param then
           ...
           if json.decode(r.body)["headers"]["Content-Type"] ~= "json/application" then
                return false, "Unexpected body answer: " .. r.body
           end
        else
